...
packet = channel*packet*noise*path_loss

t = np.arange(0, len(channel)/m.Fs, 1/m.Fs)
...
for i in range(len(packet)):
    vco.append(complex(np.cos(i*(2*np.pi*m.f)/(m.Fs)),np.sin(i*(2*np.pi*m.f)/(m.Fs))))
convolved = np.array(vco)*packet

sos = signal.butter(1,0.1*m.Fs, 'lp', fs=m.Fs, output='sos')
