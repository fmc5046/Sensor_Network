...
from LinguaFrancanode_net import (
...
import LinguaFrancanode_net as lf
try:
...
# Python class for reactor node_net
class _node_net:
    # Constructor
...
# Instantiate classes
node_net_lf = [None] * 1
node_net_nodes_lf = [None] * 6
# Start initializing node_net of class node_net
for node_net_i in range(1):
    bank_index = node_net_i
    node_net_lf[0] = _node_net(
        _bank_index = 0,
    )
    # Start initializing node_net.nodes of class Node
    for node_net_nodes_i in range(6):
        bank_index = node_net_nodes_i
        node_net_nodes_lf[node_net_nodes_i] = _Node(
            _bank_index = node_net_nodes_i,
            _num_nodes=6,
            _xi=xt[bank_index],
            _yi=yt[bank_index],
        )
